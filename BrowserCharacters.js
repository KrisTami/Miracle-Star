/*
 * Browser Characters: Modified version of Browser Ponies by Mathias Panzenböck.
 * 
 *  Please don't tell me that I steal and modified this because if Mathias sees this, he/she'll get mad for breaking a law!
 */

"use strict";

if (typeof(BrowserCharacters) !== "object") {
...
var BrowserCharacters = (function () {
	...
	extend(URL, {
		...
		fix: function (url) {
			return url.replace(/^https?:\/\/web\d?\.student\.tuwien\.ac\.at\/~e0427417\/magiland\//,"http://matia-r.github.com/Magiland/");
		}
	});
	...
	var Interaction = function Interaction (interaction) {
		...
		for (var i = 0, n = interaction.targets.length; i < n; ++ i) {
			...
			if (!has(Characters, name)) {
				console.warn("Interaction "+this.name+" of Character "+interaction.Character+
					" references non-existing Character "+name);
			}
			else {
				var Character = Characters[name];
				for (var j = 0; j < this.behaviors.length;) {
					var behavior = this.behaviors[j];
					if (has(Character.behaviors_by_name, behavior)) {
						 ++ j;
					}
					else {
	...
	Interaction.prototype = {
		reachableTargets: function (pos) {
			...
			if (n === 0)
				return targets;
			for (var i = 0; i < n; ++ i) {
				var Character = this.targets[i];
				var instance = null;
				var instance_dist = Infinity;
				for (var j = 0, m = Character.instances.length; j < m; ++ j) {
					var inst = Character.instances[j];
	...
	Behavior.prototype = {
		deref: function (property, Character) {
			var name = this[property];
			var lower_name = (name||'').toLowerCase();
			if (name && lower_name !== 'none') {
				if (has(Character.behaviors_by_name, lower_name)) {
					this[property] = Character.behaviors_by_name[lower_name];
				}
				else {
					console.warn(format("%s: Behavior %s of Character %s references non-existing behavior %s.",
						Character.baseurl, this.name, Character.name, name));
	...
	onprogress(function (resource_loaded_count, resource_count, url) {
		if (showLoadProgress || progressbar) {
			...
			progressbar.label.innerHTML = format('Loading Characters&hellip; %d%%',Math.floor(progress * 100));
	...
	var Character = function Character (Character) {
		this.baseurl = URL.join(globalBaseUrl, Character.baseurl||"");
		if (!Character.name) {
			throw new Error('Character with following base URL has no name: '+this.baseurl);
		}
		this.name = Character.name;
		this.behaviorgroups      = Character.behaviorgroups || {};
		this.all_behaviors       = [];
		...
		if (Character.categories) {
			for (var i = 0, n = Character.categories.length; i < n; ++ i) {
				this.categories.push(Character.categories[i].toLowerCase());
			}
		}
		
		if (Character.speeches) {
			for (var i = 0, n = Character.speeches.length; i < n; ++ i) {
				var speech = extend({},Character.speeches[i]);
				if (speech.files) {
					var count = 0;
				...
				if (speech.name) {
					...
					if (has(this.speeches_by_name,lowername)) {
						console.warn(format("%s: Speech name %s of Character %s is not unique.",
							this.baseurl, speech.name, Character.name));
					}
					else {
		...
		if ('behaviors' in Character) {
			for (var i = 0, n = Character.behaviors.length; i < n; ++ i) {
				var behavior = new Behavior(this.baseurl, Character.behaviors[i]);
				var lowername = behavior.name.toLowerCase();
				if (has(this.behaviors_by_name,lowername)) {
					console.warn(format("%s: Behavior name %s of Character %s is not unique.",
						this.baseurl, behavior.name, Character.name));
				}
				else {
					// semantics like Dektop Characters where the
				...
				for (var j = 0; j < speakevents.length; ++ j) {
					...
					if (speechname) {
						...
						if (has(this.speeches_by_name,speechname)) {
							...
							console.warn(format("%s: Behavior %s of Character %s references non-existing speech %s.",
								this.baseurl, behavior.name, Character.name, behavior[speakevent]));
			...
			if (this.stand_behaviors.length === 0) {
				console.warn(format("%s: Character %s has no (non-skip) non-moving behavior.", this.baseurl, this.name));
			}
	...
	Character.prototype = {
		...
		addInteraction: function (interaction) {
			...
			if (interaction.targets.length === 0) {
				console.warn("Dropping interaction "+interaction.name+" of Character "+this.name+
...
//			};
	...
	var CharacterInstance = function CharacterInstance (Character) {
	...
	CharacterInstance.prototype = extend(new Instance(), {
				...
				ondblclick: function () {
					...
					console.log(
						format('%s does %s%s for %.2f seconds, is at %d x %d and %s. See:',
							this.Character.name, this.current_behavior.name,
...
//					if (this.mouseover && (target === this.img || !descendantOf(target, this.img))) {
		...
		canDrag: function () {
			if (!this.current_behavior) {
				return this.Character.dragged_behaviors.length > 0;
			}
			else {
				var current_group = this.current_behavior.group;
				
				for (var i = 0, n = this.Character.dragged_behaviors.length; i < n; ++ i) {
					var behavior = this.Character.dragged_behaviors[i];
		...
		canMouseOver: function () {
			if (!this.current_behavior) {
				return this.Character.mouseover_behaviors.length > 0;
			}
			else {
				var current_group = this.current_behavior.group;
				for (var i = 0, n = this.Character.mouseover_behaviors.length; i < n; ++ i) {
					var behavior = this.Character.mouseover_behaviors[i];
					if (behavior.group === 0 || behavior.group === current_group) {
						return true;
					}
				}
		...
		unspawn: function () {
			...
			removeAll(this.Character.instances,this);
		...
		interact: function (currentTime,interaction,targets) {
			var Character, behavior = randomSelect(interaction.behaviors);
			this.behave(this.Character.behaviors_by_name[behavior]);
			for (var i = 0, n = targets.length; i < n; ++ i) {
				Character = targets[i];
				Character.behave(Character.Character.behaviors_by_name[behavior]);
				Character.current_interaction = interaction;
			}
...
*/
			...
			if (this.interaction_wait <= currentTime &&
					this.Character.interactions.length > 0 &&
					!this.current_interaction) {
				var sumprob = 0;
				var interactions = [];
				var interaction = null;
				for (var i = 0, n = this.Character.interactions.length; i < n; ++ i) {
					interaction = this.Character.interactions[i];
		...
		getNearestInstance: function (name) {
			...
			if (!Character) {
				...
				for (var i = 0, n = Character.instances.length; i < n; ++ i) {
					var inst = Character.instances[i];
					if (!this.loops(inst)) {
...
/*
			...
			console.log(this.Character.name+" does "+behavior.name+": "+msg+" in "+duration+
				" seconds");
*/
		},
		teleport: function () {
		...
		speakRandom: function (start_time, speak_probability) {
			...
			var filtered = [];
			var current_group = this.current_behavior.group;
			for (var i = 0, n = this.Character.random_speeches.length; i < n; ++ i) {
				var speech = this.Character.random_speeches[i];
		...
		randomBehavior: function (forceMovement) {
			...
			if (this === dragged && this.canDrag()) {
				behaviors = this.Character.dragged_behaviors;
			}
			else if (this.mouseover && this.canMouseOver()) {
				behaviors = this.Character.mouseover_behaviors;
			}
			else {
				behaviors = this.Character.random_behaviors;
			}
	...
	var EffectInstance = function EffectInstance (Character, start_time, effect) {
		this.Character       = Character;
	...
	EffectInstance.prototype = extend(new Instance(), {
		...
		updatePosition: function (currentTime, passedTime) {
			var loc, center;
			if (this.Character.facing_right) {
				loc = this.rightloc;
			...
			var CharacterRect = this.Character.topLeftRect();
			switch (loc) {
				case Locations.Top:
					pos.x += CharacterRect.x + CharacterRect.width * 0.5;
					pos.y += CharacterRect.y;
					break;
				case Locations.Bottom:
					pos.x += CharacterRect.x + CharacterRect.width * 0.5;
					pos.y += CharacterRect.y + CharacterRect.height;
					break;
				case Locations.Left:
					pos.x += CharacterRect.x;
					pos.y += CharacterRect.y + CharacterRect.height * 0.5;
					break;
				case Locations.Right:
					pos.x += CharacterRect.x + CharacterRect.width;
					pos.y += CharacterRect.y + CharacterRect.height * 0.5;
					break;
				case Locations.BottomRight:
					pos.x += CharacterRect.x + CharacterRect.width;
	...
	var getOverlay = function () {
		if (!overlay) {
			overlay = tag('div', {id: 'browser-Characters'});
		}
	...
	return {
		convertCharacter: function (ini, baseurl) {
			var rows = CharacterINI.parse(ini);
			var Character = {
				baseurl:    baseurl || "",
				behaviorgroups: {},
				behaviors:  [],
				...
				switch (type) {
					...
					case "behaviorgroup":
						...
						if (isNaN(group)) {
							...
							Character.behaviorgroups[group] = row[2];
						}
						break;
					...
					case "behavior":
									...
									if (row.length > 21) {
										...
										if (behavior.dont_repeat_animation) {
											console.warn(baseurl+': behavior '+behavior.name+
												' sets dont_repeat_animation to true, which is not supported by Browser Characters due to limitations in browsers. '+
						...
						Character.behaviors.push(behavior);
					...
					case "speak":
						...
						Character.speeches.push(speak);
						break;

					case "categories":
						Character.categories = Character.categories.concat(row.slice(1));
						break;

					default:
						console.warn(baseurl+": Unknown Character setting:",row);
				}
			}
			
			if (!('name' in Character)) {
				throw new Error('Character with following base URL has no name: '+Character.baseurl);
			}
			...
			for (var i = 0, n = effects.length; i < n; ++ i) {
				...
				if (!has(behaviors_by_name,behavior)) {
					console.warn(baseurl+": Effect "+effect.name+" of Character "+Character.name+
			...
			for (var behaviorgroup in Character.behaviorgroups) {
				has_behaviorgroups = true;
				break;
			}

			if (!has_behaviorgroups) {
				delete Character.behaviorgroups;
			}
		...
		addInteraction: function (interaction) {
			var lowername = interaction.Character.toLowerCase();
			if (!has(Characters,lowername)) {
				console.error("No such Character:",interaction.Character);
				return false;
			}
			return Characters[lowername].addInteraction(interaction);
		},
		...
		addCharacter: function (Character) {
			if (Character.ini) {
				Character = this.convertCharacter(Character.ini, Character.baseurl);
			}
			if (Character.behaviors.length === 0) {
				console.error("Character "+Character.name+" has no behaviors.");
				return false;
			}
			var lowername = Character.name.toLowerCase();
			if (has(Characters,lowername)) {
				console.error("Character "+Character.name+" already exists.");
				return false;
			}
			Characters[lowername] = new Character(Character);
			return true;
		},
		removeCharacters: function (Characters) {
			for (var i = 0, n = Characters.length; i < n; ++ i) {
		...
		removeCharacter: function (name) {
			...
			if (has(Characters,lowername)) {
				Characters[lowername].unspawnAll();
				delete Characters[lowername];
			}
		},
		spawnRandom: function (count) {
			...
			while (count > 0) {
				...
				for (var name in Characters) {
					var instcount = Characters[name].instances.length;
				...
				if (mininstcount === Infinity) {
					console.error("can't spawn random Characters because there are no Characters loaded");
					break;
				}

				var names = [];
				for (var name in Characters) {
					if (Characters[name].instances.length === mininstcount) {
						names.push(name);
					}
				}
		...
		spawn: function (name, count) {
			var lowername = name.toLowerCase();
			if (!has(Characters,lowername)) {
				console.error("No such Character:",name);
				return false;
			}
			var Character = Characters[lowername];
			if (count === undefined) {
				count = 1;
			}
			else {
			...
			var n = count;
			while (n > 0) {
				var inst = new CharacterInstance(Character);
				Character.instances.push(inst);
				if (timer !== null) {
					onload(function () {
						if (this.Character.instances.indexOf(this) === -1) return;
						instances.push(this);
		...
		unspawn: function (name, count) {
			...
			var Character = Characters[lowername];
			if (count === undefined) {
				count = Character.instances.length;
			}
			else {
				count = parseInt(count);
				if (isNaN(count)) {
			...
			if (count >= Character.instances.length) {
				Character.unspawnAll();
			}
			else {
				while (count > 0) {
					Character.instances[Character.instances.length - 1].unspawn();
					-- count;
				}
			}
			return true;
		...
		preloadAll: function () {
			for (var name in Characters) {
				Characters[name].preload();
			}
		},
		preloadSpawned: function () {
			for (var name in Characters) {
				var Character = Characters[name];
				if (Character.instances.length > 0) {
					Character.preload();
				}
			}
		},
		...
		loadConfig: function (config) {
			...
			if (config.Characters) {
				this.addCharacters(config.Characters);
			}
		...
		// currently excluding Characters and interactions
		dumpConfig: function () {
			var config = {};
			...
			// TODO: optionally dump Characters and interactions
			config.spawn = {};
			for (var name in Characters) {
				var Character = Characters[name];
				if (Character.instances.length > 0) {
					config.spawn[Character.name] = Character.instances.length;
				}
			}
		...
		toggleCharactersToBackground: function () {
			if (typeof(toggleBrowserCharactersToBackground) === "undefined") {
				alert("This website don't support bringing characters to the background.");
			}
			else {
				try {
					toggleCharactersToBackground();
				}
				catch (e) {
					alert("Error when toggling Characters to the background:\n\n"+e.name+': '+e.message);
				}
			}
		},
...
if (typeof(BrowserCharactersConfig) !== "undefined") {
	BrowserCharacters.loadConfig(BrowserCharactersConfig);
	if (BrowserCharactersConfig.oninit) {
		(function () {
			if (Array.isArray(BrowserCharactersConfig.oninit)) {
				for (var i = 0, n = BrowserCharactersConfig.oninit.length; i < n; ++ i) {
					BrowserCharactersConfig.oninit[i]();
				}
			}
			else {
				BrowserCharactersConfig.oninit();
			}
		})();
	}
}
